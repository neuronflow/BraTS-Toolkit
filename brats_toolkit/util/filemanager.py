...
def bratsNormalize(image=None, bm=None, bias=0.0001):
    ...
    if bm is None or image is None:
        print(
            "[Normalize][Error] You have to pass an image and a corresponding brain mask!"
        )
        return None
        ...
        if "brats" in fn:
            ...
            for file in files:
                if "fla.nii" == file:
                    os.rename(
                        os.path.join(subdir, file),
                        os.path.join(subdir, file.replace("fla.nii", "flair.nii")),
                    )
                if "fla.nii.gz" == file:
                    os.rename(
                        os.path.join(subdir, file),
                        os.path.join(
                            subdir, file.replace("fla.nii.gz", "flair.nii.gz")
                        ),
                    )
